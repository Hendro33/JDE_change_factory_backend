...
with TestClient(app) as client:
    ...
    backlog.propose_to_backlog(
        ...
        "customers flagged credit-exempt to be released even when the order total exceeds the credit limit, so that "
        ...
        "acceptance_criteria:\n- AC1: An exempt customer's SO order over the limit is not held.\n"
    ...
    if r.status_code != 200:
        ...
        raise SystemExit("the Architect did not route to the Technical Agent; see the recorded design")
    ...
    if not pk:
        raise SystemExit("no package prepared; see the recorded outcome")
    rev1 = pk[0]
